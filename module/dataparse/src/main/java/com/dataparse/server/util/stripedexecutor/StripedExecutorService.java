 ...
 */
package com.dataparse.server.util.stripedexecutor;
...
public class StripedExecutorService extends AbstractExecutorService {
     ...
     * is added to this HashMap.  As soon as the
    ...
    public boolean removeFromQueue(Object stripe) {
        ThreadPoolExecutor executor = (ThreadPoolExecutor) this.executor;
        BlockingQueue queue = executor.getQueue();
        return queue.removeIf(el -> {
            if(el instanceof StripedRunnable) {
                return ((StripedRunnable) el).getStripe().equals(stripe);
            }
            return false;
        });
    }
