...
import static com.google.cloud.RetryHelper.*;
import static com.google.cloud.storage.spi.StorageRpc.Option.*;
...
final class StorageImpl extends BaseService<StorageOptions> implements Storage {
  ...
  public Blob create(BlobInfo blobInfo, byte[] content, BlobTargetOption... options) {
    ...
    BlobInfo updatedInfo = blobInfo.toBuilder()
        ...
        .setCrc32c(BaseEncoding.base64().encode(
            Ints.toByteArray(Hashing.crc32().hashBytes(content).asInt())))
        .build();
  ...
  @Override
  public URL signUrl(BlobInfo blobInfo, long duration, TimeUnit unit, Map<String, String> headers, SignUrlOption... options) {
    ...
    SortedSet<String> orderedHeaderNames = new TreeSet<>(headers.keySet());
    for(String headerName: orderedHeaderNames){
      checkArgument(headerName != null, "Header name is missing");
      String headerValue = headers.get(headerName);
      checkArgument(headerValue != null, "Header value is missing");
      stBuilder.append(headerName).append(":").append(headerValue).append("\n");
    }
