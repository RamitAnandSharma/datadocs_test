...
public abstract class MetricAggregation extends Aggregation {
    ...
    /**
     * @param aggName Name of the ExactCardinalityAggregation
     * @return a new ExactCardinalityAggregation object if aggName is found within sub-aggregations of current aggregation level or null if not found
     */
    public ExactCardinalityAggregation getExactCardinalityAggregation(String aggName) {
        return jsonRoot.has(aggName) ? new ExactCardinalityAggregation(aggName, jsonRoot.getAsJsonObject(aggName)) : null;
    }
